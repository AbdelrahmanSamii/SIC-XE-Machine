...
std::map<int,std::string> addressCode;
  cout <<AddressCode<<endl;
    for(int i=0;i<sizeX;i++){
            ...
            string op = lines->at (i)[1];
        if(op=="START"){
                string temp =lines->at (i)[2];
            if(temp.size()>4){
                    char * ctemp=tocharArray(lines->at (0)[2]);
                    AddressCode = strtoul(ctemp, NULL, 16);

            }else{
                std::istringstream ( lines->at (i)[2] ) >> AddressCode;
            }
        }else if (op=="BYTE"|| op == "WORD"){
        if(op=="BYTE"){
            AddressCode+=getLength(lines->at (i)[2]);
        }else{
            AddressCode+=3;
        }

        }else if (op=="RESW"|| op == "RESB"){
            int number =getSize(lines->at (i)[2]);
        if(op=="RESB"){
            AddressCode+=number;
        }else{
        ...
        if(lines->at (j)[0]==lines->at (i)[2]){
        if(lines->at (j)[1]=="WORD"){
            flag= 3;
        }else if (lines->at (j)[1] =="BYTE"){
        flag= getLength(lines->at (j)[2]);
            }
      } else {
      flag= 0;
      }
        ...
        if(lines->at (l)[0]==lines->at (i)[2]){
        if(lines->at (l)[1]=="WORD"){
            flagtwo= 3;
        }else if (lines->at (l)[1] =="BYTE"){
        flagtwo= getLength(lines->at (l)[2]);
            }
      } else {
      flagtwo= 0;
      }
...
temp.append(result);
	for (int i = 0; i < l; i++) {
		finalCode->at(i)=toupper(temp->at(i));
	}
